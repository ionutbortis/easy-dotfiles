...
# 
# The corresponding distro folder names inside the [ <dotfiles>/private/scripts ] folder
# must match EXACTLY the SUPPORTED_DISTROS entries. 
#
# When running the [ <dotfiles>/scripts/install.sh ] script you are prompted to select the
# distro for setup and it will try to find a custom distro setup file to invoke:
# [ <dotfiles>/private/scripts/SUPPORTED_DISTRO_NAME/setup.sh ]
#
# Also, the install script will gather the list of apps to be installed by using the selected
# distro for setup. See the [ <dotfiles>/private/config/apps/config.json ] file.
#
SUPPORTED_DISTROS=( "fedora" "ubuntu" )

DEFAULT_HOST_NAME="myPC"

DEFAULT_GIT_USERNAME="my_git_username"
DEFAULT_GIT_EMAIL="my_git_email"
